...
{
    /// <summary>
    /// Custom attributes for RengaCreateColumnsComponent with Update button
    /// </summary>
    public class RengaCreateColumnsComponentAttributes : GH_ComponentAttributes
    {
        ...
        public override GH_ObjectResponse RespondToMouseDown(GH_Canvas sender, GH_CanvasMouseEvent e)
        {
            if (e.Button == System.Windows.Forms.MouseButtons.Left)
            {
                ...
                var localPoint = new PointF(e.CanvasLocation.X - Bounds.X, e.CanvasLocation.Y - Bounds.Y);
                
                // Check if click is within button bounds
                if (ButtonBounds.Contains(localPoint))
                {
                    var component = Owner as RengaCreateColumnsComponent;
                    if (component != null)
                    {
