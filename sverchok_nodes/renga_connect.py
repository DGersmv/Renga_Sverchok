...
"""
Renga Connect Node
Main node for connecting to Renga TCP server and managing data transmission
Similar to RengaConnectComponent in Grasshopper
"""
...
class SvRengaConnectNode(SverchCustomTreeNode, bpy.types.Node):
    ...
    def sv_init(self, context):
        ...
        # Note: Client object is stored internally
        # Other nodes can get port from connected node
    ...
    def process(self):
        ...
        try:
            ...
            # Use first value from input with safe extraction
            try:
            ...
            if not hasattr(self, '_client') or not hasattr(self._client, 'port') or self._client.port != port:
                self._client = renga_client.RengaConnectionClient(
                    host="127.0.0.1",
                    port=port,
                    timeout=2.0  # Shorter timeout to prevent hanging
                )
            
            # Check connection status (only if connect is True to avoid constant checks)
            if connect:
                try:
                    ...
                    if is_reachable:
                        ...
                        self.outputs['Client'].sv_set([[port]])  # Output port for other nodes
                    else:
                        self.outputs['Connected'].sv_set([[False]])
